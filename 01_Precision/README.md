...
It is one argument in five steps. Each lesson answers a question the previous one raises, so they are worth reading in order.
...
| 4 | [Machine numbers](machine_numbers/README.md) | What does a computer do with a number it cannot hold? |
| 5 | [Catastrophic cancellation](catastrophic_cancellation/README.md) | Where do the digits go when they go all at once? |

## The through-line

**Mathematics is exact.** `1/3` is exactly one third, `3,200,000 + 2` is exactly `3,200,002`, and no theorem in this chapter says otherwise. The first three lessons live in the gap between mathematics and the world — the gap opened by *measuring* something rather than counting or defining it.

That gap has a well-developed theory, and significant figures are its introductory dialect: a rule you can apply with no calculus and no error bars, taught in one class period. Lesson 3 is the real thing the dialect was translating. Lesson 4 opens a second gap, between mathematics and the machine: a number known perfectly still has to be rounded into a finite set. Lesson 5 is where the two gaps meet, and where every version of the rule, dialect and original alike, has to concede that the digits are simply gone.
