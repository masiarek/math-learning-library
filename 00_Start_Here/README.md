...
[**03_Complex_Numbers/**](../03_Complex_Numbers/README.md) — *What is a complex number, before anyone says √−1?* Three lessons: a complex number is a pair of reals and multiplication is a rule on pairs, so i² = −1 is what the rule does to the pair (0, 1); then the geometry, in which multiplying scales and turns the plane and i² = −1 is two quarter turns making a half turn; then the payoff, in which a unit point multiplied by itself walks around the circle in equal steps, so zⁿ = 1 has exactly n solutions, the roots of unity, found exactly with √3 carried as a symbol. It needs nothing but school algebra and the unit circle.
